...
def main(
    ...
    panel = Panel(init_content, title="SPYGRAPH STARTUP", box=box.ROUNDED, border_style="magenta", expand=False)
    console.print(panel)
    console.print()
    ...
    if telegraph_token:
        try:
            ...
            img_src = f"https://{api_host}/api/v1/pictures/{static_uuid}"
            ...
            try:
                ...
                telegraph_panel = Panel(telegraph_content, title="TELEGRAPH", box=box.ROUNDED, border_style="magenta", expand=False)
            ...
            error_panel = Panel(error_content, title="ERROR", box=box.ROUNDED, border_style="magenta", expand=False)
    ...
    config_panel = Panel(config_content, title="SERVER", box=box.ROUNDED, border_style="magenta", expand=False)
    console.print(config_panel)
    console.print()
    
    queue = Queue()
    
    api_process = Process(target=run_api, args=(queue, host, port, ssl_cert, ssl_key, static_uuid), daemon=False)
    ...
    def handle_interrupt(sig, frame):
        ...
        if shutdown_state["interrupt_count"] == 1:
            ...
            shutdown_content = Text("  ◆ Graceful shutdown initiated\n  ◦ Press Ctrl+C again to force\n", style="bold magenta")
            shutdown_panel = Panel(shutdown_content, title="SHUTDOWN", box=box.ROUNDED, border_style="magenta", expand=False)
            console.print(shutdown_panel)
        else:
            force_content = Text("  ◆ Force shutdown\n", style="bold magenta")
            force_panel = Panel(force_content, title="FORCED", box=box.ROUNDED, border_style="magenta", expand=False)
