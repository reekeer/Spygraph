...
def main() -> None:
  ...
  (________)   (____/(__)  (__/  \___/(__\_)\_/\_/(__)  \_)(_/ [/bold magenta]""")
    ...
    create_account_parser = subparsers.add_parser("create_account", help="Create a new Telegraph account")
    create_account_parser.add_argument("--name", type=str, required=True, help="Account short name (5-32 characters)")
    create_account_parser.add_argument("--author-name", type=str, default=None, help="Optional author name")
    create_account_parser.add_argument("--author-url", type=str, default=None, help="Optional author URL")
    create_account_parser.add_argument(
        "--domain-graph", type=str, default="telegra.ph", help="Telegraph domain or mirror (default: telegra.ph)"
    )

    run_parser = subparsers.add_parser("run", help="Start API server with Telegraph logging")
    ...
    run_parser.add_argument("--domain-graph", type=str, default="telegra.ph", help="Telegraph domain or mirror (default: telegra.ph)")
    run_parser.add_argument(
        "--account-name", type=str, default="SpyGraph", help="Account name for new Telegraph account (default: SpyGraph)"
    )
    run_parser.add_argument("--title", type=str, default="SpyGraph Target", help="Title for the Telegraph page (default: SpyGraph Target)")
    run_parser.add_argument("--content", type=str, default=None, help="Path to HTML/TXT content file for Telegraph page")
    run_parser.add_argument("--author_name", type=str, default="SpyGraph", help="Author name for Telegraph page (default: SpyGraph)")
    run_parser.add_argument(
        "--ssl-cert", type=str, default=None, help="Path to SSL certificate file (certbot: /etc/letsencrypt/live/domain/fullchain.pem)"
    )
    run_parser.add_argument(
        "--ssl-key", type=str, default=None, help="Path to SSL private key file (certbot: /etc/letsencrypt/live/domain/privkey.pem)"
    )
    ...
    if args.command == "create_account":
        try:
            ...
            result = Grapher.create_graph_account(
                short_name=args.name, author_name=args.author_name, author_url=args.author_url, domain_graph=args.domain_graph
            )
        ...
        try:
            ...
            if not telegraph_token:
                ...
                result = Grapher.create_graph_account(short_name=args.account_name, domain_graph=args.domain_graph)
